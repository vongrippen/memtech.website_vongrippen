...
_webring.insertLink = function(selector, href, text) {
    try {
        document.querySelectorAll(selector)[0].setAttribute('href', href);
    }
    catch(err) {
        // Error :(
    }
...
_webring.load = function() {
    ...
    rq.open('GET', 'http://memtech.website/data/user_stats.json', true);
    ...
    rq.onreadystatechange = function() {
        if (this.readyState === 4) {
            if (this.status >= 200 && this.status < 400) {
                ...
                _webring.insertLink('.webring_prev', 'http://memtech.website/~' + _webring.users[_webring.prevUserIndex])
                _webring.insertLink('.webring_rand', 'http://memtech.website/~' + _webring.users[_webring.randomIndex])
                _webring.insertLink('.webring_next', 'http://memtech.website/~' + _webring.users[_webring.nextUserIndex])
            }
            else {
                // Error :(
            }
